...
describe("Dip Delegator", () => {
  ...
  let forestCreator: anchor.web3.Keypair,
    treeCreator: anchor.web3.Keypair,
    stakeholder1: anchor.web3.Keypair,
    stakeholder2: anchor.web3.Keypair;
  let voteMint: PublicKey;

  before(async () => {
    [id, forestCreator, treeCreator, stakeholder1, stakeholder2] =
      await createKeypairs(program.provider.connection, 4);
    provider = new anchor.AnchorProvider(
      program.provider.connection,
      new anchor.Wallet(forestCreator),
      {}
    );
    ...
    voteMint = (
      await mintToken(provider, forestCreator, stakeholder1.publicKey)
    ).mint;
  ...
  it("Create a tree and win a node auction", async () => {
    ...
    await provider.connection.confirmTransaction(
      await program.methods
        .createForest(id.publicKey, forestCreator.publicKey, treeCreationFee)
        .accounts(
          getCreateForestAccounts(
            id.publicKey,
            voteMint,
    ...
    expect(
      (
        await getAccount(
          provider.connection,
          getAssociatedTokenAddressSync(voteMint, forestCreator.publicKey)
        )
      ).amount.toString()
